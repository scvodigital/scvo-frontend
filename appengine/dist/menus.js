            ...
            switch (_a.label) {
                case 0:
                    ...
                    if (stripDomains && stripDomains.length > 0) {
                        stripDomains = stripDomains.map(function (domain) { return domain.replace(/\./g, '\\.'); });
                ...
                case 5:
                    ...
                    menusArray.forEach(function (menu) {
                        if (menu.slug) {
                            menus[menu.slug] = menu.items.map(function (item) { return transformWpMenu(item, domainRegex); });
                        }
                    });
...
function transformWpMenu(wpMenuItem, domainRegex) {
    var children = !wpMenuItem.children ? [] : wpMenuItem.children.map(function (child) { return transformWpMenu(child, domainRegex); });
    var metaData = {};
