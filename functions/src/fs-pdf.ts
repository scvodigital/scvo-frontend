...
function generatePdf(image, funds, title: string = 'Funding Scotland', subtitle: string = null) {
    return new Promise((resolve, reject) => {
        try{
...
function uriFix(uri, defaultProtocol = 'https'){
    if(!uri) return null;
...
function getFunds(ids){
    return new Promise((resolve, reject) => {
        ...
        request.get(getOptions, (err, resp, body) => {
            ...
            if(!body.hits || body.hits.total === 0 || !body.hits.hits) return reject(body);
            var funds = body.hits.hits.map((hit) => { return hit._source; });
            resolve(funds);
        });
    });
}
